...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (database && process.env.RESEND_API_KEY) {
      try {
        const { runEmailSendPipeline } = await import("@/server/worker/pipelines/email-send");
    ...
    if (database && process.env.POSTIZ_API_KEY) {
      try {
        const { runSocialSendPipeline } = await import("@/server/worker/pipelines/social-send");
    ...
    if (database && process.env.YOUTUBE_REFRESH_TOKEN && process.env.S3_BUCKET) {
      try {
        ...
        const sid = rows[0]?.id;
        if (sid) {
          const { uploadDailySermonToYouTube } = await import("@/server/worker/pipelines/sermon-to-youtube");
